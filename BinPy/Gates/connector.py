...
class Connector:
    ...
    * trigger
    ...
    def set_logic(self, val):
        ...
        self.trigger()
        # All set functions ultimately call this. So one trigger here should suffice.
...
class Bus:
    ...
    def set_width(self, width, *connectors):
        ...
        # Use this method sparingly. It would be good practice to keep Bus objects of fixed size.
        ...
        if width == self._width:
            ...
            if len(connectors) == width - self._width:
                self.bus += [ ( conn if isinstance(conn, Connector) else Connector() ) for conn in connectors ]
    ...
    def set_logic_all(self, *values):
        ...
        if type(values[0]) is int:
            ...
            word = values[0]
            word = bin(int(word, 2))[2:].zfill(self._width)  # This will convert '11', '0011' and '0b0011' to '0011'
            
        elif type(values[0]) is list:
            word = values[0]
            str_int_bool = lambda o : str(int(bool(o)))
            # This is done to convert Connector elements to logic states or to ensure the list passed is binary
        ...
        if len(word) != self._width:
            ...
            raise Exception("ERROR: Input width is not the same as that of the bus")
    ...
    def set_voltage_all(self, *values):
        ...
        if type(values[0]) is list:
            ...
            # 1. Converts 5 to 5.0
            # 2. When inputs are connectors it extracts the voltage data from them.
    ...
    def __eq__(self, val):
        ...
        bus_values = self.get_voltage_all() if self.analog else self.get_logic_all()
        
        if isinstance(val, Bus):
            return bus_values == ( val.get_voltage_all() if self.analog else val.get_logic_all() )

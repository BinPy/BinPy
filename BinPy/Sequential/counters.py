...
class BinaryCounter(Counter):
    ...
    def __init__(self,clk):
        ...
        self.ff = [ TFlipFlop(self.t,self.enable,self.clk,self.out[0]), TFlipFlop(self.t,self.enable,self.out[0],self.out[1]) ]
...
class NBitRippleCounter(Counter):
    ...
    def __init__(self,bits,clock_connector):
        ...
        for i in range(1,bits):
            self.ff.append(TFlipFlop(self.t,self.enable,self.out[i-1],self.out[i]))

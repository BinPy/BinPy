...
class BinaryCounter(Counter):
    ...
    def __init__(self, clk,set=Connector(0),reset=Connector(0),data = 0):
        ...
        self.ff[1] = TFlipFlop(self.t, self.enable, self.clk, self.out[1],self.outinv[1],self.set,self.reset)
        self.ff[0] = TFlipFlop(self.t, self.enable, self.out[1], self.out[0],self.outinv[0],self.set,self.reset)
...
class NBitRippleCounter(Counter):
    ...
    def __init__(self, bits, clock_connector,set=Connector(0),reset=Connector(0),data = 0):
        ...
        self.ff[self.bits-1] = TFlipFlop(self.t, self.enable, self.clk, self.out[self.bits-1],self.outinv[self.bits-1],self.set,self.reset)

        for i in range(self.bits-1):
            self.ff[i] = TFlipFlop(self.t, self.enable, self.out[i+1], self.out[i],self.outinv[i],self.set,self.reset)
...
class NBitDownCounter(Counter):
    ...
    def __init__(self, bits, clock_connector,set=Connector(0),reset=Connector(0),data = 0):
        ...
        for i in range(self.bits-1):
            self.ff[i] = TFlipFlop(self.t, self.enable, self.outinv[i+1], self.out[i],self.outinv[i],self.set,self.reset)
...
class DecadeCounter(Counter):
    ...
    def __init__(self, clock_connector,set=Connector(0),reset=Connector(0),data = 0):
        ...
        self.ff[3] = TFlipFlop(self.t, self.enable, self.clk, self.out[3], self.outinv[3], self.set, self.reset)
        self.ff[2] = TFlipFlop(self.t, self.enable, self.out[3], self.out[2], self.outinv[2], self.set, self.reset)
        self.ff[1] = TFlipFlop(self.t, self.enable, self.out[2], self.out[1], self.outinv[1], self.set, self.reset)
        self.ff[0] = TFlipFlop(self.t, self.enable, self.out[1], self.out[0], self.outinv[0], self.set, self.reset)

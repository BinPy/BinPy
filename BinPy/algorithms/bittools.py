...
class BitTools(object):
    ...
    # DEV NOTE : BitArray is the class name so pep8 naming ( bit_array ) is not used
    ...
    def to_BitArray(input_data, bits, signed=False):
        ...
        if type(input_data) not in [str, int, BitArray]:
            raise Exception ("ERROR: Input must be given as integer or binary strings or bitarray objects")
        ...
        if type(input_data) == int:
            ...
            # Sign is decided by the "signed" parameter or - in the input_data string
            ...
            # Next priority to 2s complement signed binary explicitly mentined as signed
            else:
            ...
            raise Exception ("ERROR: Input  must be given as binary strings or integers.")
    ...
    # to the purpose of this class and to give a method similar to to_signed_int(binary_str)

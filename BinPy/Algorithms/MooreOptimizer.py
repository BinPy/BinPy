...
class StateMachineSolver(object):
    ...
    class InternalOptimizer(object):
        ...
        def __init__(self, state_word_len, variables):
            ...
            variable_names = map(
                lambda i: 'S%d' % i, xrange(self.state_word_len))
...
class StateMachineOptimizer(object):
...
class StateMachineOptimizer_Random(StateMachineOptimizer):
    ...
    def optimize(self, tries=1000):
        ...
        for counter in xrange(tries):
            if counter & 0xff == 0:
                sys.stderr.write(
                    'Tried %d random permutations out of %d.\r' % (counter,
                                                                   total))
...
def main():
    ...
    opti = StateMachineOptimizer_Random(
        state_tran, state_word_len, variables, outputs, print_best=True)
    opti.optimize()
